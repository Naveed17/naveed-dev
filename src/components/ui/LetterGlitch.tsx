...
const LetterGlitch = ({
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      resizeTimeout = setTimeout(() => { cancelAnimationFrame(animationRef.current as number); resizeCanvas(); animateRef.current(); }, 100);
    };
  ...
  return (
    <div className="relative w-full h-full bg-[#101010] overflow-hidden">
      ...
      {outerVignette && <div className="absolute top-0 left-0 w-full h-full pointer-events-none bg-[radial-gradient(circle,rgba(16,16,16,0)_60%,rgba(16,16,16,1)_100%)]" />}
      {centerVignette && <div className="absolute top-0 left-0 w-full h-full pointer-events-none bg-[radial-gradient(circle,rgba(0,0,0,0.8)_0%,rgba(0,0,0,0)_60%)]" />}
    </div>
  );
};
