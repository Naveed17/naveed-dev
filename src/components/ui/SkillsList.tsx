...
const skills: Record<string, string[]> = {
  ...
  "UI/UX Design & Prototyping": ["UI design with Figma & Canva", "UX research & improvements", "Prototyping for websites & mobile apps"],
};
        ...
        {Object.entries(skills).map(([category, items]) => (
          <li key={category} className="w-full">
            <div
              ...
              className="md:w-100 w-full bg-surface rounded-2xl text-left transition-all border border-(--white-icon-tr) cursor-pointer overflow-hidden hover:bg-surface-hover"
            >
              <div className="flex items-center gap-3 p-4">
                {CategoryIcons[category]}
                <div className="flex items-center gap-2 grow justify-between">
                  <div className="min-w-0 max-w-50 md:max-w-none overflow-hidden">
                    <span className="block truncate text-(--white) text-lg">{category}</span>
                  </div>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                    className={`w-6 h-6 text-(--white) transform transition-transform shrink-0 ${openItem === category ? "rotate-180" : ""}`}>
                    <path d="M11.9999 13.1714L16.9497 8.22168L18.3639 9.63589L11.9999 15.9999L5.63599 9.63589L7.0502 8.22168L11.9999 13.1714Z" />
                  </svg>
                </div>
              </div>
              <div className={`transition-all duration-300 px-4 ${openItem === category ? "max-h-125 pb-4 opacity-100" : "max-h-0 opacity-0"}`}>
