...
const techStack = [
  { desc: "Built with", name: "Next.js", icon: "/svg/next.svg", alt: "Next.js Logo" },
  { desc: "Styled with", name: "TailwindCSS", icon: "/svg/tailwindcss.svg", alt: "TailwindCSS Logo" },
  { desc: "Deployed on", name: "Vercel", icon: "/svg/vercel.svg", alt: "Vercel Logo" },
];
    ...
    <footer className="w-full py-12 border-t border-border">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col lg:items-start items-center space-y-6 gap-9">
            <div className="flex space-x-6 sm:space-x-8">
              {socialLinks.map((link) => (
                <a key={link.label} href={link.href} target="_blank" className="flex flex-col items-center group" aria-label={link.label}>
            ...
            <div className="grid grid-cols-1 gap-3 w-full max-w-xs py-6">
              {techStack.map((tech) => (
                <div key={tech.name} className="flex items-center justify-center lg:justify-normal space-x-3">
                  <span className="text-(--white-icon) text-sm">{tech.desc}</span>
                  <Image src={tech.icon} alt={tech.alt} width={20} height={20} className={`object-contain opacity-50 ${tech.name === "Vercel" ? "dark:invert invert-0" : "invert dark:invert-0"}`} />
                  <span className="text-(--white-icon) text-sm">{tech.name}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-12 pt-8 border-t border-border">
          <p className="text-center text-sm text-(--white-icon)">
            <span className="block sm:inline">
              Copyright © <span suppressHydrationWarning>{currentYear}</span>{" "}
              <a href="https://github.com/Naveed17">Naveed Ahmed</a>. All rights reserved.
            </span>
          </p>
        </div>
      </div>
